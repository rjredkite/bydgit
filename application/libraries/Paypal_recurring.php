...
https://github.com/kidino/paypal_recurring

How to install and use
----------------------

- Store this file (Paypal_recurring.php) into the application/library folder
...
$this->load->library('paypal_recurring', $paypal_credentials);

********************************************/
class Paypal_recurring {
	// set these using the constructor
	private $USE_PROXY = false;
	private $PROXY_HOST = 'http://www.breedyourdog.com';
	private $PROXY_PORT = '8443';
	private $API_username = "breedyourdog_api1.hotmail.co.uk";
	private $API_password = "AFZCHLVKCZBFJYNS";
	private $API_signature = "AW8Bt5CFoXq0rnxdLpML5ykcLf7EAUBRtmCsKwEeSEf-G.4XdoV20xNp";
	...
	*/
	function set_ec($ec_data = array(), $custom_data = false) {
		...
		if ($custom_data === false) {
			...
			$nvpstr.= "&L_BILLINGAGREEMENTDESCRIPTION0=" . urlencode($ec_data['desc']);
	...
	*/
	function do_ec(
		...
		if (isset($ec_details['amount'])) {
			$nvpstr.= '&PAYMENTREQUEST_0_CUSTOM=' . urlencode($ec_details['custom_id']);
		}
	...
	function do_rec(
		...
		$nvpstr = '';
		if (isset($rec_details['token'])) {
			$nvpstr.= '&TOKEN=' . urlencode($rec_details['token']);
		}
		if (isset($ec_details['payer_id'])) {
			$nvpstr.= '&PAYERID=' . urlencode($ec_details['payer_id']);
		}
		if (isset($rec_details['profile_start_date'])) {
			$nvpstr.= '&PROFILESTARTDATE=' .urldecode($rec_details['profile_start_date']);
		}
		if (isset($rec_details['description'])) {
			$nvpstr.= '&DESC=' .urldecode($rec_details['description']);
		}
		if (isset($rec_details['billing_period'])) {
			$nvpstr.= '&BILLINGPERIOD=' .urldecode($rec_details['billing_period']);
		}
		if (isset($rec_details['billing_frequency'])) {
			$nvpstr.= '&BILLINGFREQUENCY=' .urldecode($rec_details['billing_frequency']);
		}
		if (isset($rec_details['total_billing_cycles'])) {
			//$nvpstr.= '&TOTALBILLINGCYCLES=' .urldecode($rec_details['total_billing_cycles']);
		}
		if (isset($rec_details['amount'])) {
			$nvpstr.= '&AMT=' .urldecode($rec_details['amount']);
		} 
		if (isset($rec_details['currency_code'])) {
			$nvpstr.= '&CURRENCYCODE=' .urldecode($rec_details['currency_code']);
		}
		if (isset($rec_details['max_failed_payments'])) {
			$nvpstr.= '&MAXFAILEDPAYMENTS=' .urldecode($rec_details['max_failed_payments']);
		}

		if (isset($rec_details['paymentrequest_item_category'])) {
			//$nvpstr.= '&L_PAYMENTREQUEST_0_ITEMCATEGORY0=' .urldecode($rec_details['paymentrequest_item_category']);
		}
		if (isset($rec_details['paymentrequest_name'])) {
			//$nvpstr.= '&L_PAYMENTREQUEST_0_NAME0=' .urldecode($rec_details['paymentrequest_name']);
		}
		if (isset($rec_details['paymentrequest_amount'])) {
			//$nvpstr.= '&L_PAYMENTREQUEST_0_AMT0=' .urldecode($rec_details['paymentrequest_amount']);
		}
		if (isset($rec_details['paymentrequest_qty'])) {
			//$nvpstr.= '&L_PAYMENTREQUEST_0_QTY0=' .urldecode($rec_details['paymentrequest_qty']);
		}
		if (isset($ec_details['IPN_URL'])) {
			$nvpstr.= '&NOTIFYURL=' . urlencode($ec_details['IPN_URL']);
		}
	...
	function hash_call($methodName, $nvpStr) {
		...
		curl_setopt($ch, CURLOPT_PROXYUSERPWD, "admin:kaziGo4a1"); //username:pass 
	...
	*/
	...
	function change_subscription_status( $profile_id, $action ) {
	    ...
	    $api_request = 'USER=' . urlencode( 'breedyourdog_api1.hotmail.co.uk' )
	                .  '&PWD=' . urlencode( 'AFZCHLVKCZBFJYNS' )
	                .  '&SIGNATURE=' . urlencode( 'AW8Bt5CFoXq0rnxdLpML5ykcLf7EAUBRtmCsKwEeSEf-G.4XdoV20xNp' )
	                .  '&VERSION=93'
